...
if(isset($_POST['donate_amount']) &&
	intval($_POST['donate_amount']) &&
	isset($_POST['payment_method']) &&
	($_POST['payment_method'] == 'stripe' || $_POST['payment_method'] == 'check_or_wire')){

	// above if should be cleaned up based on validation

	$debug = True;
	$live = True;

	// generate email
	// should validate all (see ica.art/sign-up)

	$msg  = "New York Consolidated has received a donation.\r\n\r\n";
	$msg .= "NAME : " . $_POST['name'] . "\r\n";
	$msg .= "NAME TO ACKNOWLEDGE : " . $_POST['name_to_acknowledge'] . "\n";
	$msg .= "MAILING ADDRESS : " . $_POST['mailng_address'] . "\n";
	$msg .= "EMAIL : " . $_POST['email'] . "\n";
	$msg .= "TELEPHONE : " . $_POST['telephone'] . "\n";
	$msg .= "AMOUNT : " . $_POST['donate_amount'] . "\n";
	$msg .= "PAYMENT METHOD : " . $_POST['payment_method'] . "\n";
	$msg = wordwrap($msg,70);
	...
	if ($live)
		mail("mia@n-y-c.org","New York Consolidated donation",$msg);
	...
	if ($uri[2] != 'success' && $uri[2] != 'canceled' && $uri[2] != 'submitted'){
		...
		?><div id="donate-form">
			...
			<form action='<?= $requestclean; ?>' method='post' class='no-break'>
				...
				<input type="text" name="name_to_acknowledge" placeholder="(Name to acknowledge)">
				<textarea type="text" name="mailing_address" placeholder="Mailing address" rows="4" cols="100"></textarea>
				<input type="text" name="email" placeholder="Email">
				<input type="text" name="telephone" placeholder="Telephone">
				<select id="amount" name="donate_amount">
				...
				<select id="payment" name="payment_method">
					...
					<option value="check_or_wire"> Please send me the details for a check/wire transfer</option>
				</select>
				<input type="submit">
